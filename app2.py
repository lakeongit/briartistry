...
def register_service_provider():
    data = request.get_json()
    if not validate_provider_data(data):
        return jsonify({'message': 'Invalid service provider registration data'}), 400
    ...
    try:
        ...
        conn.close()
        return jsonify({'message': 'Service provider registration failed', 'error': str(e)}), 500
...
def validate_provider_data(data):
    # Validate required fields
    if 'username' not in data or 'password' not in data or 'name' not in data or 'email' not in data or 'contact_details' not in data or 'service_offerings' not in data:
        return False
